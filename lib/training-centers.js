 ...
 * - "section" : Section sportive scolaire (collège / lycée)
 *
 * Pour ajouter une section sportive : copier le format ci-dessous, choisir un id unique.
 */
...
export const TRAINING_CENTERS = [
  ...
  // ═══════════ SECTIONS SPORTIVES (à compléter par Milan) ═══════════
  // Ajoute ici les sections sportives scolaires (collèges / lycées) que tu veux référencer.
  // Format : { id: "section_xxx", label: "Nom du lycée / collège", city: "Ville", type: "section" }
];
...
export const CENTER_TYPE_META = {
  ...
  section: { label: "Section sportive", color: "#10B981", icon: "🎓", priority: 3 },
};
...
export function getCentersGrouped() {
  return {
    ...
    section: TRAINING_CENTERS.filter((c) => c.type === "section"),
  };
}
